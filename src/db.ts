...
export function createTask(
  ...
  db.prepare(
    `
    INSERT INTO scheduled_tasks (id, group_folder, chat_jid, prompt, schedule_type, schedule_value, context_mode, next_run, status, created_at, creator_sender, creator_privilege)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `,
  ).run(
    task.id,
    ...
    task.status,
    task.created_at,
    task.creator_sender ?? null,
    task.creator_privilege ?? 'owner',
  );
}
